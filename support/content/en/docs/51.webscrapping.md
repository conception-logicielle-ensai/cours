...
Le web scraping désigne les techniques d’extraction du contenu des sites internet. Cette une pratique que l'on envisage lorsque l'on a accès a des données utiles publiquement mais qu'il ne nous est pas fourni de fichiers produits ou d'API pour les traiter. 
...
console: elle permet d'executer du javascript sur la page
- Un inspecteur : permet de scanner les éléments et de récupérer des informations sur celles ci.
...
On peut l'utiliser aussi bien pour la réalisation de tests fonctionnels : `je me connecte a tel doctolib, je m'attends a trouver 10 profils différents de médecins dans l'affichage`
