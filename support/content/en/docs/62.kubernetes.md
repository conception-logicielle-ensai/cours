...
Constatez que dans le sous dossier : exemples-cours/cours-6/kubernetes/app il y a différents fichiers ̀`yaml` de configuration de ressources pour kubernetes.
...
**pour relancer**
=> Soit vous supprimez tout refaites tout : `kubectl delete -f exemples-cours/cours-6/kubernetes/app/` puis `kubectl apply -f exemples-cours/cours-6/kubernetes/app/`
=> Soit vous supprimez le pod : `kubectl get pods` puis en notant lepodtrouvésvpchangezmoi le nom du pod `kubectl delete pod lepodtrouvésvpchangezmoi`
